...
# 1. Create a folder "results" in the directory where this code is run. The output files are written there.
# 2. Run (ctrl+enter) the cell at the very bottom of this notebook to load packages and define all functions required.
# 3. Run "parameter specification".
# 4. Run "simulation".
...
NumberOfSims=1;   #how many independent simulations are run for this parameter set
...
for sim in 1:NumberOfSims
    ...
    M[1,1]=M[1,2]=M[N,N]=M[N,N-1]=1    #set (next) diagonal to 1
    for i=2:N-1
        M[i,i-1]=M[i,i]=M[i,i+1]=1
    end


    #First Bond manually for programming syntax reasons ('bonds')
...
for i in 1:N
    for j in NB[i]
      if abs(i-j)>1
        ...
        deleteat!(NB[j],findall((in)(i), NB[j]))
      end
    end 
end
